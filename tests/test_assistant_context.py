...
def test_context_redacts_secrets_and_never_returns_sql_or_credentials(monkeypatch, tmp_path):
    ...
    context = AssistantContextService(state_path=db_path, source_columns_reader=lambda *_args: None).build(run_id="run-1")
...
def test_context_reader_never_mutates_app_state(monkeypatch, tmp_path):
    ...
    AssistantContextService(state_path=db_path, source_columns_reader=lambda *_args: None).build(run_id="run-1")
...
def test_actual_silver_run_id_resolves_exact_silver_context(monkeypatch, tmp_path):
    """Test A: Actual Silver run ID resolves exact Silver context."""
    db_path = tmp_path / "app_state.sqlite"
    monkeypatch.setenv("AURUM_APP_STATE_DB", str(db_path))
    with get_connection() as conn:
        conn.execute("INSERT INTO projects (id, name, created_at, updated_at) VALUES ('p1', 'p1', 'x', 'x')")
        conn.execute("INSERT INTO data_connections (id, project_id, type, name, host, port, database_name, username, status, created_at, updated_at) VALUES ('conn-1', 'p1', 'postgresql', 'c1', 'localhost', 5432, 'db', 'user', 'active', 'x', 'x')")
        conn.execute(
            """
            INSERT INTO generated_sql_review
                (run_id, table_name, sql_text, planned_changes_json, created_at, status,
                 candidate_schema, generator_provenance, project_id, connection_id, source_identity_json)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("silver-exact-100", "orders", "SELECT...", json.dumps({"rules": ["rule 1"]}),
             "2026-07-28T03:00:00Z", "PENDING", "silver", "silver_gen_v1", "p1", "conn-1",
             json.dumps({"schema": "public", "relation": "orders"})),
        )
        conn.commit()

    service = AssistantContextService(state_path=db_path)
    context = service.build(run_id="silver-exact-100")

    assert context["run"]["id"] == "silver-exact-100"
    assert context["run"]["mode"] == "silver"
    assert context["source"]["schema"] == "public"
    assert context["source"]["relation"] == "orders"
    assert context["gold"]["run_id"] is None


def test_no_run_id_returns_empty_insufficient_context(monkeypatch, tmp_path):
    """Test B: No run ID returns empty/insufficient context."""
    db_path = tmp_path / "app_state.sqlite"
    _seed_state(monkeypatch, db_path)

    service = AssistantContextService(state_path=db_path)
    context = service.build(run_id=None)
...
def test_conn_a_orders_cannot_resolve_conn_b_orders(monkeypatch, tmp_path):
    """Test D: conn-A/source_a.orders cannot resolve conn-B/source_b.orders."""
    db_path = tmp_path / "app_state.sqlite"
    monkeypatch.setenv("AURUM_APP_STATE_DB", str(db_path))
    with get_connection() as conn:
        conn.execute("INSERT INTO projects (id, name, created_at, updated_at) VALUES ('p1', 'p1', 'x', 'x')")
        ...
        conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("ing-b", "p1", "conn-B", "db_b", "source_b", "orders", "{}", "bronze", "bronze_orders_b", "{}", "READY", "x", "x"),
        )
        conn.execute(
            """
            INSERT INTO generated_sql_review
                (run_id, table_name, sql_text, planned_changes_json, created_at, status, candidate_schema, generator_provenance, project_id, connection_id, source_identity_json)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("silver-conn-a", "orders", "SELECT...", "{}", "2026-07-28T02:00:00Z", "PENDING", "silver", "gen", "p1", "conn-A", json.dumps({"schema": "source_a", "relation": "orders"})),
        )
        conn.commit()

    service = AssistantContextService(state_path=db_path)
    context = service.build(run_id="silver-conn-a")

    assert context["connection"]["id"] == "conn-A"
    assert context["bronze"]["relation"] is None
    assert context["bronze"]["ingest_id"] is None


def test_same_connection_bare_orders_ambiguity_cannot_be_selected(monkeypatch, tmp_path):
    """Test E: Same connection: source_a.orders vs source_b.orders cannot be selected using bare 'orders'."""
    db_path = tmp_path / "app_state.sqlite"
    monkeypatch.setenv("AURUM_APP_STATE_DB", str(db_path))
    with get_connection() as conn:
        conn.execute("INSERT INTO projects (id, name, created_at, updated_at) VALUES ('p1', 'p1', 'x', 'x')")
        ...
        conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("ing-1", "p1", "conn-A", "db_a", "source_a", "orders", "{}", "bronze", "bronze_orders_a", "{}", "READY", "x", "x"),
        )
        conn.execute(
            """
            INSERT INTO bronze_ingest_authority
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("ing-2", "p1", "conn-A", "db_a", "source_b", "orders", "{}", "bronze", "bronze_orders_b", "{}", "READY", "x", "x"),
        )
        conn.execute(
            """
            INSERT INTO generated_sql_review
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("silver-ambiguous-orders", "orders", "SELECT...", "{}", "2026-07-28T02:00:00Z", "PENDING", "silver", "gen", "p1", "conn-A", json.dumps({"table": "orders"})),
        )
        conn.commit()

    service = AssistantContextService(state_path=db_path)
    context = service.build(run_id="silver-ambiguous-orders")

    assert context["source"]["schema"] is None
    assert context["bronze"]["relation"] is None


def test_multiple_historical_silver_runs_no_latest_guessing_when_run_id_absent(monkeypatch, tmp_path):
    """Test F: Multiple historical Silver runs: no latest-run guessing when run ID is absent."""
    db_path = tmp_path / "app_state.sqlite"
    monkeypatch.setenv("AURUM_APP_STATE_DB", str(db_path))
    with get_connection() as conn:
        conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("silver-run-1", "orders", "SELECT...", "{}", "2026-07-28T01:00:00Z", "PENDING", "silver", "gen"),
        )
        conn.execute(
            """
            INSERT INTO generated_sql_review
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("silver-run-2", "orders", "SELECT...", "{}", "2026-07-28T02:00:00Z", "PROMOTED", "silver", "gen"),
        )
        conn.commit()
...
def test_missing_silver_run_never_becomes_gold_context(monkeypatch, tmp_path):
    """Test G: Missing Silver run never becomes Gold context."""
    db_path = tmp_path / "app_state.sqlite"
    monkeypatch.setenv("AURUM_APP_STATE_DB", str(db_path))
    with get_connection() as conn:
        conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("gold-run-1", "orders", "CREATE...", "{}", "2026-07-28T02:00:00Z", "PROMOTED", "gold", "gen"),
        )
        conn.execute(
            """
            INSERT INTO gold_security_state
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("gold-run-1", "v1", "v1", "Gold req", "{}", "gold", "orders", "cand", "cand_orders", "gen", "v1", "{}", "r1"),
        )
        conn.commit()

    service = AssistantContextService(state_path=db_path)
    context = service.build(run_id="non-existent-silver-run")
...
def test_exact_gold_run_id_still_resolves_gold_context(monkeypatch, tmp_path):
    """Test H: Exact Gold run ID still resolves Gold context."""
    db_path = tmp_path / "app_state.sqlite"
    monkeypatch.setenv("AURUM_APP_STATE_DB", str(db_path))
    with get_connection() as conn:
        conn.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("gold-exact-777", "daily_metrics", "CREATE...", json.dumps({"metric": "SUM(val)"}), "2026-07-28T02:00:00Z", "PROMOTED", "gold_candidates", "gold_gen"),
        )
        conn.execute(
            """
            INSERT INTO gold_security_state
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            ("gold-exact-777", "v1", "v1", "Daily summary", json.dumps({"sources": []}), "gold", "daily_metrics", "gold_candidates", "daily_metrics_cand", "gold_gen", "v1", "{}", "rev777"),
        )
        conn.commit()

    service = AssistantContextService(state_path=db_path)
    context = service.build(run_id="gold-exact-777")

    assert context["run"]["id"] == "gold-exact-777"
    assert context["gold"]["run_id"] == "gold-exact-777"
    assert context["gold"]["status"] == "PROMOTED"
    assert context["gold"]["business_requirement"] == "Daily summary"


def test_validation_runs_backed_exact_run_still_works(monkeypatch, tmp_path):
    """Test I: validation_runs-backed exact run still works."""
    db_path = tmp_path / "app_state.sqlite"
    _seed_state(monkeypatch, db_path)
