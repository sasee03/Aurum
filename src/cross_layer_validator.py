...
def build_business_impact(loader: DataLoader, cfg: AurumDatasetConfig | None = None) -> dict:
    ...
    if not (
        ...
        loader.table_exists(f"{bronze_table}") and loader.table_exists(f"{gold_metrics}")
        ...
        return {
            ...
            "detail": f"Expected baseline not available ({bronze_table}/{gold_metrics} missing).",
    ...
    expected = loader.scalar(
        ...
        f"FROM {cfg.tables.bronze} WHERE {_valid_bronze_predicate(cfg)}"
