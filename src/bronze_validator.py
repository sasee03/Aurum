...
def b1_source_to_bronze_count(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    source = loader.count(f"{raw_table}") if loader.table_exists(f"{raw_table}") else None

    bronze = loader.count(f"{bronze_table}")

    if source is None:

        status, detail = WARN, f"No {cfg.tables.raw} source table to reconcile against."
...
def b4_required_columns(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    cols = loader.columns(f"{bronze_table}")
    ...
    return CheckResult(
        ...
        evidence_query=f"SELECT * FROM {cfg.tables.bronze} LIMIT 0",
...
def b6_mandatory_nulls(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    for col in mandatory_not_null:
        ...
        if col in cols:
            ...
            null_counts[col] = int(
                ...
                loader.scalar(f"SELECT COUNT(*) FROM {bronze_table} WHERE {col} IS NULL")
    ...
    return CheckResult(
        ...
        evidence_query=(
            ...
            f"SELECT COUNT(*) FROM {cfg.tables.bronze} WHERE {cfg.columns.primary_key} IS NULL"
...
def b7_negative_values(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    neg_qty = int(loader.scalar(f"SELECT COUNT(*) FROM {bronze_table} WHERE {qty_col} < 0"))

    neg_price = int(

        loader.scalar(f"SELECT COUNT(*) FROM {bronze_table} WHERE {price_col} < 0")
    ...
    return CheckResult(
        ...
        evidence_query=f"SELECT COUNT(*) FROM {cfg.tables.bronze} WHERE {qty_col} < 0",
...
def b8_duplicates(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    total = loader.count(f"{bronze_table}")
    ...
    return CheckResult(
        ...
        evidence_query=(
            ...
            f"SELECT {key_cols}, COUNT(*) FROM {cfg.tables.bronze} "
...
def b9_invoice_date_parse(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    return CheckResult(
        ...
        evidence_query=(
            ...
            f"SELECT COUNT(*) FROM {cfg.tables.bronze} "
