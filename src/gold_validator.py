...
def g1_revenue_reconciliation(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    silver_rev = float(loader.scalar(f"SELECT SUM({revenue_col}) FROM {silver_table}") or 0)
    ...
    return CheckResult(
        ...
        evidence_query=f"SELECT SUM({revenue_col}) FROM {cfg.tables.silver}",
...
def g2_order_count_reconciliation(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    silver_orders = int(
        ...
        loader.scalar(
            ...
            f"SELECT COUNT(DISTINCT {order_id_expression}) FROM {silver_table}"
    ...
    return CheckResult(
        ...
        evidence_query=f"SELECT COUNT(DISTINCT {order_id_expression}) FROM {cfg.tables.silver}",
...
def g3_customer_count_reconciliation(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    silver_customers = int(
        ...
        loader.scalar(f"SELECT COUNT(DISTINCT {cust_col}) FROM {silver_table}") or 0
    ...
    return CheckResult(
        ...
        evidence_query=f"SELECT COUNT(DISTINCT {cust_col}) FROM {cfg.tables.silver}",
...
def g4_average_order_value(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    return CheckResult(
        ...
        evidence_query=f"SELECT {rev_metric} / {orders_metric} FROM {cfg.tables.gold.metrics}",
...
def g5_revenue_vs_baseline(
    ...
    if not stats or stats["std"] == 0:
        ...
        return CheckResult(
            ...
            evidence_query=f"SELECT {revenue_metric} FROM {cfg.tables.gold.metrics}",
...
def g6_country_revenue_reconciliation(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    if not loader.table_exists(f"{gold_country}"):
        ...
        return CheckResult(
            ...
            detail=f"No {cfg.tables.gold.country_revenue} table to reconcile.",
    ...
    mismatch = int(
        ...
        loader.scalar(
            ...
            WITH silver_by_country AS (
                ...
                FROM {silver_table} GROUP BY "{geo_col}"
            )
            SELECT COUNT(*) FROM {gold_country} g
    ...
    return CheckResult(
        ...
        evidence_query=f"SELECT {geo_col}, SUM({rev_col}) FROM {cfg.tables.silver} GROUP BY {geo_col}",
...
def g7_metrics_cardinality(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    n = int(loader.scalar(f"SELECT COUNT(*) FROM {gold_metrics}") or 0)
    ...
    return CheckResult(
        ...
        evidence_query=f"SELECT COUNT(*) FROM {cfg.tables.gold.metrics}",
...
def g8_mandatory_metrics_not_null(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    cols = loader.columns(f"{gold_metrics}")
    ...
    return CheckResult(
        ...
        evidence_query=f"SELECT COUNT(*) FROM {cfg.tables.gold.metrics} WHERE {rev_metric} IS NULL",
...
def g9_revenue_per_customer(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    return CheckResult(
        ...
        evidence_query=(
            ...
            f"SELECT {revenue_metric} / NULLIF({customers_metric}, 0) FROM {cfg.tables.gold.metrics}"
...
def g10_country_coverage(loader: DataLoader, cfg: Optional[AurumDatasetConfig] = None) -> CheckResult:
    ...
    if not loader.table_exists(f"{gold_country}"):
        ...
        return CheckResult(
            ...
            detail=f"No {cfg.tables.gold.country_revenue} table to reconcile country coverage.",
    ...
    silver_countries = int(
        ...
        loader.scalar(f'SELECT COUNT(DISTINCT "{geo_col}") FROM {silver_table}') or 0

    )

    gold_countries = int(

        loader.scalar(f"SELECT COUNT(*) FROM {gold_country}") or 0
    ...
    detail = (
        ...
        if match
            ...
            f"{cfg.tables.gold.country_revenue} has {gold_countries:,} rows."
    ...
    return CheckResult(
        ...
        evidence_query=f"SELECT COUNT(DISTINCT {geo_col}) FROM {cfg.tables.silver}",
