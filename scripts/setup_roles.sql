...
BEGIN
    ...
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'aurum_generated_sql') THEN
        CREATE ROLE aurum_generated_sql WITH LOGIN PASSWORD 'aurum_generated_sql';
    END IF;
...
GRANT USAGE, CREATE ON SCHEMA silver TO aurum_generated_sql;
GRANT USAGE, CREATE ON SCHEMA gold TO aurum_generated_sql;
...
GRANT aurum_generated_sql TO aurum_promotion;
DO $$
BEGIN
